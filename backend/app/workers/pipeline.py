...
class TelegramPipeline:
    ...
    async def _record_non_signal(self, source: TelegramSource, message: TelegramMessage, body: str) -> None:
        try:
            ...
            self._audit("TELEGRAM_MESSAGE_UNRECOGNIZED", "telegram_message", str(message.id), None, {"source_id": str(source.id)})
            return
    ...
    async def _route(self, source: TelegramSource, signal: Signal, normalized: Any) -> None:
        ...
        for account in accounts:
            ...
            if settings is None or account.trading_mode != TradingMode.SHADOW:
                await self._decision(signal, account, Decision.REJECTED, ["Only fully configured SHADOW accounts are executed by this worker"])
                continue
            if profile is None:
            ...
            rules = evaluate_rules(RuleContext(
                ...
                remaining_daily_buffer=max(Decimal("0"), Decimal(settings.maximum_daily_loss_internal) + (Decimal(stat.realized_pnl + stat.floating_pnl) if stat else Decimal("0"))),
            ...
            if blockers:
                ...
                continue
            shadow_accounts.append(self._shadow_account(account, settings, permitted, daily.risk_multiplier, max(Decimal("0.01"), drawdown.remaining_buffer), current_exposure))
            model_accounts.append(account)
        run = self.shadow.route(normalized, shadow_accounts)
        by_id = {str(account.id): account for account in model_accounts}
        for decision in run.decisions:
            account = by_id[decision.account_id]
            rule_snapshot = RuleSnapshot(payload={"automation_permitted": next(a.automation_permitted for a in shadow_accounts if a.account_id == decision.account_id)})
            ...
            await self.db.flush()
            execution_id = f"{signal.id}:{account.id}" if decision.decision == Decision.APPROVED else None
            ...
            order_payload = {"symbol": managed.symbol, "direction": managed.direction.value, "volume": str(managed.original_volume), "stop_loss": str(managed.stop_loss)}
            self.db.add_all([
                Order(execution_id=execution_id, correlation_id=signal.correlation_id or self.correlation_id, signal_id=signal.id, account_id=account.id, broker_order_id=managed.position_id, status="FILLED", request=order_payload, response={"virtual": True, "fill_price": str(managed.entry_price)}),
            ...
            self._audit("SHADOW_POSITION_OPENED", "position", str(position.id), None, {"execution_id": execution_id, "classification": normalized.risk_classification.value})
            self._notify("INFO", "SHADOW position opened", f"{account.name}: {managed.direction.value} {normalized.symbol} {managed.original_volume} lots.")
        signal.status = SignalStatus.EXECUTED if any(item.decision == Decision.APPROVED for item in run.decisions) else SignalStatus.REJECTED
    ...
    def _notify(self, severity: str, subject: str, body: str) -> None:
        self.db.add(Notification(channel="DASHBOARD", correlation_id=self.correlation_id, severity=severity, subject=subject, body=body, status="QUEUED"))
